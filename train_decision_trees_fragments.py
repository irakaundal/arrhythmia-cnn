...
from read_data_other_classifiers_fragments import read_data
...
def run():
    ...
    clf = DecisionTreeClassifier(criterion="gini",
                                 random_state=100, max_depth=10, min_samples_leaf=17)
    ...
    with open("Results_other_classifiers.txt", "a+") as f:
        f.write("Accuracy for Decision Tree Classifier with gini criterion, 100 random state, max depth=10, minimum samples leaf=17 and with fragments without 10-fold cross validation is: "+str(np.mean(acc)*100) + "%"+"\n")
    f.close()
