...
def run():
    ...
    with open("Results_other_classifiers.txt", "a+") as f:
        f.write(
            "Accuracy for Decision Tree Classifier with gini criterion, 100 random state, max depth=10, minimum samples leaf=17 with 1d data without 10-fold cross validation is: " + str(
                np.mean(acc) * 100) + "%" + "\n")
    f.close()
